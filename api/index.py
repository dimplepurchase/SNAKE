...
@app.before_request
def check_timeout():
    if request.endpoint and 'static' not in request.endpoint and request.endpoint not in ['login', 'register']:
        if 'user_id' in session:
            last_active = session.get('last_active')
            timeout_minutes = session.get('idle_timeout', 15)
            if last_active and time.time() - last_active > (timeout_minutes * 60):
                session.clear()
                return redirect(url_for('login', msg="Session expired due to inactivity."))
...
LOGIN_TEMPLATE = '''<!DOCTYPE html><html><head><title>System Access</title>
...
<style>
    ...
    {% if msg %}.alert { color: var(--danger); font-size: 0.9em; text-align: center; margin-bottom: 15px; font-weight: bold; }{% endif %}
    ...
    #game-over-msg { display: none; color: red; text-align: center; margin-top: 20px; font-size: 1.2em; font-family: 'Poppins', sans-serif; font-weight: bold; cursor: pointer; }
</style>
</head><body>
    <div id="game-wrapper" {% if demo_mode == False and game_enabled != 0 %}style="display: block;"{% else %}style="display: none;"{% endif %}>
        <div class="hud">
            <div id="timeDisplay">Time: 0s</div>
            {% if demo_mode %}
            <select id="speedCtrl" onchange="speedSetting = parseInt(this.value); updateSpeed();" style="background: #111; color: #0f0; border: 1px solid #0f0; border-radius: 4px; font-family: monospace; font-size: 0.8em; outline: none; padding: 2px;">
                <option value="-15">Slowest</option>
                <option value="0" selected>Normal</option>
                <option value="15">Fastest</option>
            </select>
            {% endif %}
            <div id="scoreDisplay">Score: 0</div>
        </div>
        <canvas id="gameCanvas" width="400" height="400"></canvas>
        <div id="game-over-msg" onclick="location.reload();">Game Over.<br>Tap to Restart.</div>
        <div class="controls">
    ...
    <div id="login-container" {% if demo_mode or game_enabled == 0 %}style="display: block;"{% endif %}>
        <h2>System Access</h2>
    ...
    <script>
        {% if demo_mode == False and game_enabled != 0 %}
        const canvas = document.getElementById('gameCanvas');
        const ctx = canvas.getContext('2d');
        const grid = 20;
        
        let reqApples = {{ game_apples }};
        let targetCorner = "{{ game_corner }}"; 
        let speedSetting = {{ game_speed }}; 
        let delayMs = 120;
        
        function updateSpeed() {
            // Speed logic: Base 120ms. Speed +10 makes it 40ms. Speed -10 makes it 200ms.
            delayMs = 120 - (speedSetting * 8);
            if (delayMs < 30) delayMs = 30; // Max fast limit
        }
        updateSpeed();
        ...
        let lastTime = performance.now(); // High precision timer for stable mobile speed
        ...
        function loop(timestamp) {
            ...
            // Strictly enforce timing for smooth mobile and desktop parity
            if (timestamp - lastTime < delayMs) return;
            lastTime = timestamp;
            ...
            // TARGET CORNER CHECK
            let atTarget = false;
            if(targetCorner === 'br' && snake.x === canvas.width - grid && snake.y === canvas.height - grid) atTarget = true;
            if(targetCorner === 'bl' && snake.x === 0 && snake.y === canvas.height - grid) atTarget = true;
            if(targetCorner === 'tr' && snake.x === canvas.width - grid && snake.y === 0) atTarget = true;
            if(targetCorner === 'tl' && snake.x === 0 && snake.y === 0) atTarget = true;
...
TRASH_TEMPLATE = '''<!DOCTYPE html><html><head><title>Trash / Recycle Bin</title>''' + BASE_STYLE + '''</head><body>
    <div class="container">''' + NAVBAR_HTML + '''
        <div class="card" style="padding: 0;">
            ...
            <form action="/bulk_action" method="POST">
                <div style="padding: 10px 20px; border-bottom: 1px solid var(--border); background: #fdf2f8;">
                    <select name="action" required style="width: auto; display: inline-block;">
                        <option value="">-- Select Bulk Action --</option>
                        <option value="restore">♻️ Restore Selected</option>
                        <option value="hard_delete">🔥 Permanently Delete Selected</option>
                    </select>
                    <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Apply this action to all selected entries?');">Apply</button>
                </div>
                <table style="width: 100%; border: none;"><tr><th style="padding-left: 20px; width: 40px;"><input type="checkbox" onclick="toggleCheckboxes(this)" style="cursor:pointer; width:16px; height:16px;"></th><th>Date & Time</th><th>Category / Detail</th><th style="text-align: right;">Amount</th><th style="text-align: center;">Action</th></tr>
                    {% for t in trashed %}<tr>
                        <td style="padding-left: 20px;"><input type="checkbox" name="selected_links" value="{{ t.link_id }}" style="cursor:pointer; width:16px; height:16px;"></td>
                        ...
                        <td style="text-align: center;">
                            ...
                            <a href="/hard_delete_voucher/{{ t.link_id }}" class="btn btn-sm" style="background:#dc2626; color:white; margin-left:5px;" onclick="return confirm('Permanently delete?');">🔥</a>
                        </td>
...
MAIN_LEDGER_TEMPLATE = '''<!DOCTYPE html><html><head><title>Main Cash Book Ledger</title>''' + BASE_STYLE + '''</head><body>
    <div class="container">''' + NAVBAR_HTML + '''
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 10px;">
            ...
            <select onchange="window.location.href='?filter='+this.value" style="width: 200px; border-color: var(--primary); font-weight: bold;">
                <option value="all" {% if current_filter == 'all' %}selected{% endif %}>All Entries</option>
                <option value="in" {% if current_filter == 'in' %}selected{% endif %}>Credit (+ IN) Only</option>
                <option value="out" {% if current_filter == 'out' %}selected{% endif %}>Debit (- OUT) Only</option>
            </select>
        </div>
        ...
        <div class="card" style="padding: 0; overflow-x: auto;">
            <form action="/bulk_action" method="POST">
                <input type="hidden" name="source" value="main_ledger">
                {% if session.get('can_edit') == 1 or session.get('role') == 'superadmin' %}
                <div style="padding: 10px 25px; border-bottom: 1px solid var(--border); background: #f8fafc;">
                    <select name="action" required style="width: auto; display: inline-block;">
                        <option value="">-- Bulk Action --</option>
                        <option value="trash">🗑️ Move Selected to Trash</option>
                    </select>
                    <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Move selected entries to trash?');">Apply</button>
                </div>
                {% endif %}
                <table style="width: 100%; min-width: 800px;">
                    <thead><tr>
                        {% if session.get('can_edit') == 1 or session.get('role') == 'superadmin' %}
                        <th style="padding-left: 25px; width: 40px;"><input type="checkbox" onclick="toggleCheckboxes(this)" style="cursor:pointer; width:16px; height:16px;"></th>
                        {% endif %}
                        <th {% if not session.get('can_edit') %}style="padding-left: 25px;"{% endif %}>Date & Time</th><th>Mode/Category</th><th style="width: 45%;">Bill No / Details & Link</th><th style="text-align: right;">Amount</th>
                        {% if session.get('can_edit') == 1 or session.get('role') == 'superadmin' %}<th style="text-align: center;">Act</th>{% endif %}
                    </tr></thead>
                    <tbody>
                        {% for txn in txns %}<tr>
                            {% if session.get('can_edit') == 1 or session.get('role') == 'superadmin' %}
                            <td style="padding-left: 25px;"><input type="checkbox" name="selected_links" value="{{ txn.link_id }}" style="cursor:pointer; width:16px; height:16px;"></td>
                            {% endif %}
                            <td {% if not session.get('can_edit') %}style="padding-left: 25px;"{% endif %}><span style="font-weight: 500;">{{ txn.date }}</span><br><span style="color: #6b7280; font-size: 0.85em;">{{ txn.time }}</span></td>
                            ...
                            <td style="text-align: right;">
                                ...
                                {% if txn.type in ['expense', 'dasti_out', 'batch_ledger_out', 'dasti_voucher_out'] %}<span class="badge badge-out">- ₹{{ "{:,.2f}".format(txn.amount) }}</span>
                                {% else %}<span class="badge badge-in">+ ₹{{ "{:,.2f}".format(txn.amount) }}</span>{% endif %}
                            </td>
                            {% if session.get('can_edit') == 1 or session.get('role') == 'superadmin' %}
                            <td style="text-align: center;"><a href="/edit/transactions/{{ txn.id }}" class="btn btn-sm" style="background:#f59e0b;color:white;">✏️</a> <a href="/delete/transactions/{{ txn.id }}" class="btn btn-sm btn-danger" onclick="return confirm('Move to Trash?');">🗑️</a></td>
                            {% endif %}
                        </tr>{% else %}<tr><td colspan="6" style="text-align:center; color:#9ca3af; padding: 40px;">No entries found.</td></tr>{% endfor %}
                    </tbody>
                </table>
            </form>
        </div>
...
PERSON_ACCOUNT_TEMPLATE = '''<!DOCTYPE html><html><head><title>Account: {{ person.name }}</title>''' + BASE_STYLE + '''</head><body>
    <div class="container">''' + NAVBAR_HTML + '''
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 10px;">
            <div>
                <a href="/persons" class="btn-outline" style="text-decoration: none; padding: 8px 15px; border-radius: 8px; font-size: 0.9em; margin-right: 15px;">← Back</a>
                <h2 style="margin: 0; display: inline-block; font-size: 1.6em;">Person: <span style="color: var(--primary);">{{ person.name }}</span></h2>
            </div>
            <select onchange="window.location.href='?filter='+this.value" style="width: 200px; border-color: var(--primary); font-weight: bold;">
                <option value="all" {% if current_filter == 'all' %}selected{% endif %}>All Entries</option>
                <option value="in" {% if current_filter == 'in' %}selected{% endif %}>Advance (+ IN) Only</option>
                <option value="out" {% if current_filter == 'out' %}selected{% endif %}>Slips (- OUT) Only</option>
            </select>
        </div>
        <div class="stats-grid">
        ...
        <div class="card" style="padding: 0; overflow-x: auto;">
            <form action="/bulk_action" method="POST">
                <input type="hidden" name="source" value="person_{{ person.id }}">
                {% if session.get('can_edit') == 1 or session.get('role') == 'superadmin' %}
                <div style="padding: 10px 25px; border-bottom: 1px solid var(--border); background: #f8fafc;">
                    <select name="action" required style="width: auto; display: inline-block;">
                        <option value="">-- Bulk Action --</option><option value="trash">🗑️ Move Selected to Trash</option>
                    </select>
                    <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Move selected entries to trash?');">Apply</button>
                </div>
                {% endif %}
                <table style="width: 100%; min-width: 800px;">
                    ...
                    <tbody>
                        {% for txn in txns %}<tr>
                            ...
                            {% if session.get('can_edit') == 1 or session.get('role') == 'superadmin' %}
                            <td style="text-align: center;"><a href="/edit/person_ledger/{{ txn.id }}" class="btn btn-sm" style="background:#f59e0b;color:white;">✏️</a> <a href="/delete/person_ledger/{{ txn.id }}" class="btn btn-sm btn-danger" onclick="return confirm('Move to Trash?');">🗑️</a></td>
                            {% endif %}
                        </tr>{% else %}<tr><td colspan="6" style="text-align:center; color:#9ca3af; padding: 40px;">No entries found.</td></tr>{% endfor %}
                    </tbody>
                </table>
...
DASTI_ACCOUNT_TEMPLATE = '''<!DOCTYPE html><html><head><title>Dasti Account: {{ person.name }}</title>''' + BASE_STYLE + '''</head><body>
    <div class="container">''' + NAVBAR_HTML + '''
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 10px;">
            <div>
                <a href="/dasti_ledger" class="btn-outline" style="text-decoration: none; padding: 8px 15px; border-radius: 8px; font-size: 0.9em; margin-right: 15px;">← Back</a>
                <h2 style="margin: 0; display: inline-block; font-size: 1.6em;">Dasti: <span style="color: #0ea5e9;">{{ person.name }}</span></h2>
            </div>
            <select onchange="window.location.href='?filter='+this.value" style="width: 200px; border-color: var(--primary); font-weight: bold;">
                <option value="all" {% if current_filter == 'all' %}selected{% endif %}>All Entries</option>
                <option value="in" {% if current_filter == 'in' %}selected{% endif %}>Advance (+ IN) Only</option>
        ...
        <div class="card" style="padding: 0; overflow-x: auto;">
            <form action="/bulk_action" method="POST">
                <input type="hidden" name="source" value="dasti_{{ person.id }}">
                {% if session.get('can_edit') == 1 or session.get('role') == 'superadmin' %}
                <div style="padding: 10px 25px; border-bottom: 1px solid var(--border); background: #f8fafc;">
                    <select name="action" required style="width: auto; display: inline-block;">
                        <option value="">-- Bulk Action --</option><option value="trash">🗑️ Move Selected to Trash</option>
                ...
                <table style="width: 100%; min-width: 800px;">
                    <thead><tr>
                        ...
                        <th style="padding-left: 25px; width: 40px;"><input type="checkbox" onclick="toggleCheckboxes(this)" style="cursor:pointer; width:16px; height:16px;"></th>
                        {% endif %}
                        <th {% if not session.get('can_edit') %}style="padding-left: 25px;"{% endif %}>Date & Time</th><th>Mode/Category</th><th style="width: 45%;">Dasti Detail & Link</th><th style="text-align: right;">Amount</th>
                        {% if session.get('can_edit') == 1 or session.get('role') == 'superadmin' %}<th style="text-align: center;">Act</th>{% endif %}
                    </tr></thead>
                    <tbody>
                        {% for txn in txns %}<tr>
                            ...
                            {% if session.get('can_edit') == 1 or session.get('role') == 'superadmin' %}
                            <td style="text-align: center;"><a href="/edit/dasti_ledger/{{ txn.id }}" class="btn btn-sm" style="background:#f59e0b;color:white;">✏️</a> <a href="/delete/dasti_ledger/{{ txn.id }}" class="btn btn-sm btn-danger" onclick="return confirm('Move to Trash?');">🗑️</a></td>
                            {% endif %}
                        </tr>{% else %}<tr><td colspan="6" style="text-align:center; color:#9ca3af; padding: 40px;">No entries found.</td></tr>{% endfor %}
                    </tbody>
                </table>
...
def get_game_settings(firm_id):
    doc = db.collection('settings').document(f'game_{firm_id}').get()
    if doc.exists: return doc.to_dict()
    return {'apples': 4, 'corner': 'br', 'speed': 0, 'enabled': 1}
...
def index():
    ...
    # Strictly Serial (Ascending) Order everywhere
    incomes.sort(key=lambda x: (x.get('date', ''), x.get('time', ''), x.get('created_at', 0)), reverse=False)
    expenses.sort(key=lambda x: (x.get('date', ''), x.get('time', ''), x.get('created_at', 0)), reverse=False)
...
def main_ledger():
    ...
    all_txns = [{'id': doc.id, **doc.to_dict()} for doc in db.collection('transactions').where('user_id', '==', firm_id).where('deleted', '==', 0).stream()]
    all_txns.sort(key=lambda x: (x.get('date', ''), x.get('time', ''), x.get('created_at', 0)), reverse=False) # Serial Ascending
    ...
    if filter_val == 'in':
        all_txns = [t for t in all_txns if t.get('type') not in ['expense', 'dasti_out', 'batch_ledger_out', 'dasti_voucher_out']]
    elif filter_val == 'out':
        all_txns = [t for t in all_txns if t.get('type') in ['expense', 'dasti_out', 'batch_ledger_out', 'dasti_voucher_out']]

    return render_template_string(MAIN_LEDGER_TEMPLATE, txns=all_txns, balance=balance, total_in=total_in, total_out=total_out, total_dasti=total_dasti, total_dasti_vouchers=total_dasti_vouchers, current_filter=filter_val, username=session['username'], active_page='main_ledger')
...
def person_account(person_id):
    ...
    if filter_val == 'in':
        txns = [t for t in txns if t.get('type') == 'advance']
    elif filter_val == 'out':
        txns = [t for t in txns if t.get('type') == 'settlement']

    txns.sort(key=lambda x: (x.get('date', ''), x.get('time', ''), x.get('created_at', 0)), reverse=False) # Serial Ascending
        
    return render_template_string(PERSON_ACCOUNT_TEMPLATE, person=person, txns=txns, balance=(advances - settlements), advances=advances, settlements=settlements, current_filter=filter_val, username=session['username'], active_page='persons')
...
def dasti_account(person_id):
    ...
    return render_template_string(DASTI_ACCOUNT_TEMPLATE, person=person, txns=txns, balance=(advances - settlements), advances=advances, settlements=settlements, current_filter=filter_val, username=session['username'], active_page='dasti_ledger')
...
def bulk_action():
    ...
    if not selected_links or not action: return redirect(request.referrer or url_for('index'))
    ...
    for link_id in selected_links:
        for collection in ['transactions', 'person_ledger', 'dasti_ledger']:
            ...
            for d in docs:
                if action == 'trash': d.reference.update({'deleted': 1})
                elif action == 'restore': d.reference.update({'deleted': 0})
                elif action == 'hard_delete': d.reference.delete()
...
def login():
    ...
    game_apples, game_corner, game_speed, game_enabled = 4, 'br', 0, 1
    
    try:
        first_user = next(db.collection('users').limit(1).stream(), None)
        if first_user:
            f_id = first_user.to_dict().get('firm_id')
            gset = get_game_settings(f_id)
            game_apples = gset.get('apples', 4)
            game_corner = gset.get('corner', 'br')
            game_speed = gset.get('speed', 0)
            game_enabled = gset.get('enabled', 1)
    except: pass
    
    if request.method == 'POST':
        uname = request.form['username'].lower().strip()
        pwd = request.form['password'].lower().strip()
        
        users_stream = db.collection('users').stream()
        user_doc, user = None, None
        
        for doc in users_stream:
            data = doc.to_dict()
            if data.get('username', '').lower().strip() == uname:
                if check_password_hash(data['password'], pwd) or check_password_hash(data['password'], request.form['password']):
                    user_doc, user = doc, data
                    break
        ...
        if user_doc:
            ...
            session['idle_timeout'] = user.get('idle_timeout', 15)
            session['last_active'] = time.time()
            ...
            return render_template_string(LOGIN_TEMPLATE, msg="Invalid Username or Password.", demo_mode=False, game_apples=game_apples, game_corner=game_corner, game_speed=game_speed, game_enabled=game_enabled)
            
    return render_template_string(LOGIN_TEMPLATE, demo_mode=demo_mode, game_apples=game_apples, game_corner=game_corner, game_speed=game_speed, game_enabled=game_enabled)
...
def update_game_settings():
    ...
    db.collection('settings').document(f"game_{session['firm_id']}").set({
        'apples': int(request.form.get('apples', 4)),
        'corner': request.form.get('corner', 'br'),
...
def manage_users():
    ...
    gset = get_game_settings(session['firm_id'])
    return render_template_string(USERS_TEMPLATE, users=users, game_settings=gset, username=session['username'], active_page='users')
...
def approvals():
    ...
    vouchers = [{'id': doc.id, **doc.to_dict()} for doc in db.collection('transactions').where('user_id', '==', firm_id).where('status', '==', status_filter).where('deleted', '==', 0).stream()]
    vouchers.sort(key=lambda x: (x.get('date', ''), x.get('time', ''), x.get('created_at', 0)), reverse=False) # Serial order
    ...
    return render_template_string(APPROVALS_TEMPLATE, vouchers=vouchers, pending=vouchers, current_status=status_filter, approvers=approvers, username=session['username'], active_page='approvals')
...
def bulk_approve_selected():
    ...
    is_reject = request.form.get('reject_btn') == '1'
...
def trash():
    ...
    trashed.sort(key=lambda x: (x.get('date', ''), x.get('time', ''), x.get('created_at', 0)), reverse=False) # Serial Order
    return render_template_string(TRASH_TEMPLATE, trashed=trashed, username=session['username'], active_page='trash')

@app.route('/register', methods=['GET', 'POST'])
def register():
    if has_users(): return "Setup complete.", 403
    if request.method == 'POST':
        new_user_ref = db.collection('users').document()
        user_id = new_user_ref.id
        
        db.collection('settings').document(f"game_{user_id}").set({'apples': 4, 'corner': 'br', 'speed': 0, 'enabled': 1})
        
        new_user_ref.set({
            'username': request.form['username'], 
            'password': generate_password_hash(request.form['password'].lower().strip()),
            'firm_name': request.form['firm_name'],
            'firm_id': user_id,
            'role': 'superadmin',
            'can_approve': 1, 'can_edit': 1, 'can_express_cashout': 1, 'can_view_reports': 1, 'can_view_trash': 1,
            'idle_timeout': 60
        })
        ...
        if opening_balance > 0:
            now = datetime.now()
            db.collection('transactions').add({'user_id': user_id, 'date': now.strftime('%Y-%m-%d'), 'time': now.strftime('%H:%M'), 'payment_mode': 'Cash', 'category': 'General', 'description': 'Opening Balance', 'type': 'income', 'amount': opening_balance, 'link_id': uuid.uuid4().hex[:12], 'status': 'approved', 'approved_by': session['username'], 'deleted': 0, 'created_at': time.time()})
        return redirect(url_for('index'))
...
def add_user():
    ...
    db.collection('users').add({
        ...
        'password': generate_password_hash(request.form['new_password'].lower().strip()),
        'firm_name': session['firm_name'],
        'firm_id': session['firm_id'],
        'role': request.form['role'],
        'idle_timeout': int(request.form.get('idle_timeout', 15)),
...
def edit_user(uid):
    ...
    if request.method == 'POST':
        ...
        if new_pw: update_data['password'] = generate_password_hash(new_pw.lower().strip())
        doc_ref.update(update_data)
        return redirect(url_for('manage_users'))
    return render_template_string(EDIT_USER_TEMPLATE, edit_user={'id': uid, **user_data}, username=session['username'], active_page='users')

@app.route('/add_express', methods=['POST'])
def add_express():
    if 'user_id' not in session: return redirect(url_for('login'))
    db.collection('transactions').add({'user_id': session['firm_id'], 'date': request.form['date'], 'time': request.form['time'], 'payment_mode': 'Cash', 'category': 'General', 'description': request.form['description'], 'type': request.form['type'], 'amount': float(request.form['amount']), 'link_id': uuid.uuid4().hex[:12], 'status': 'pending', 'approved_by': '', 'deleted': 0, 'created_at': time.time()})
    return redirect(request.referrer or url_for('index'))

@app.route('/add_transfer', methods=['POST'])
def add_transfer():
    if 'user_id' not in session: return redirect(url_for('login'))
    firm_id, date_val, time_val, direction, person_id = session['firm_id'], request.form['date'], request.form['time'], request.form['direction'], request.form['person_id']
    ...
    person_name, link_id, txn_status, approver = person_doc.to_dict().get('name', ''), uuid.uuid4().hex[:12], 'pending', ''
...
def add_batch_unified():
    if 'user_id' not in session: return redirect(url_for('login'))
    firm_id, date_val, time_val, mode, txn_nature = session['firm_id'], request.form['date'], request.form['time'], request.form['payment_mode'], request.form['txn_nature']
    primary_account_raw, new_account_name = request.form['primary_account'], request.form.get('new_account_name', '').strip()
    ...
    account_type, primary_id, person_name, txn_status, approver = 'main', None, '', 'pending', ''
    ...
    for i in range(len(descs)):
        if amts[i].strip() and float(amts[i]) >= 0:
            ...
            if account_type == 'main':
                db.collection('transactions').add({**base_txn, 'description': desc, 'type': 'income' if txn_nature == 'receive_cash' else 'expense'})
...
def reports():
    ...
    collection_name, pid_filter, pid_field = 'transactions', None, None
    if report_account.startswith('person_'):
        collection_name, pid_filter, pid_field = 'person_ledger', report_account.split('_')[1], 'person_id'
    elif report_account.startswith('dasti_'):
        collection_name, pid_filter, pid_field = 'dasti_ledger', report_account.split('_')[1], 'dasti_person_id'
    ...
    results = [r for r in [doc.to_dict() for doc in query.stream()] if (not start_date or r.get('date', '') >= start_date) and (not end_date or r.get('date', '') <= end_date) and (not category or r.get('category', '') == category)]
    results.sort(key=lambda x: (x.get('date', ''), x.get('time', ''), x.get('created_at', 0)), reverse=False) # Serial order
    
    total_in = sum(float(r.get('amount', 0)) for r in results if r.get('type') in ('income', 'settlement', 'dasti_voucher_in'))
    if report_account == 'main': total_out = sum(float(r.get('amount', 0)) for r in results if r.get('type') in ('expense', 'dasti_out', 'dasti_voucher_out'))
    else: total_out = sum(float(r.get('amount', 0)) for r in results if r.get('type') in ('expense', 'advance', 'dasti_out', 'dasti_voucher_out'))
    
    persons = [{'id': doc.id, **doc.to_dict()} for doc in db.collection('persons').where('user_id', '==', firm_id).stream()]
    dasti_persons = [{'id': doc.id, **doc.to_dict()} for doc in db.collection('dasti_persons').where('user_id', '==', firm_id).stream()]
    
    return render_template_string(REPORTS_TEMPLATE, results=results, total_in=total_in, total_out=total_out, categories=get_categories(firm_id), persons=persons, dasti_persons=dasti_persons, start_date=start_date, end_date=end_date, category=category, report_account=report_account, username=session['username'], active_page='reports')
...
def export_reports():
    ...
    start_date, end_date, category, report_account = request.args.get('start_date', ''), request.args.get('end_date', ''), request.args.get('category', ''), request.args.get('report_account', 'main')
    collection_name, pid_filter, pid_field = 'transactions', None, None
    if report_account.startswith('person_'): collection_name, pid_filter, pid_field = 'person_ledger', report_account.split('_')[1], 'person_id'
    elif report_account.startswith('dasti_'): collection_name, pid_filter, pid_field = 'dasti_ledger', report_account.split('_')[1], 'dasti_person_id'
    ...
    def generate():
        data = StringIO(); writer = csv.writer(data); writer.writerow(('Date', 'Time', 'Mode', 'Category', 'Description', 'Type', 'Amount (INR)', 'Approved By'))
...
def edit_entry(table_name, row_id):
    ...
    if link_id:
        for collection in ['transactions', 'person_ledger', 'dasti_ledger']:
            ...
            if found: linked_docs[collection] = (found[0].id, found[0].to_dict())

    txn_doc, person_doc, dasti_doc = linked_docs.get('transactions'), linked_docs.get('person_ledger'), linked_docs.get('dasti_ledger')

    if person_doc: current_account_type, current_primary_id = 'person', person_doc[1].get('person_id', '')
    elif dasti_doc: current_account_type, current_primary_id = 'dasti', dasti_doc[1].get('dasti_person_id', '')
    else: current_account_type, current_primary_id = 'main', ''
    ...
    if request.method == 'POST':
        ...
        if category and category not in get_categories(firm_id): db.collection('categories').add({'firm_id': firm_id, 'name': category})

        date_val, time_val, mode, amount, desc = request.form['date'], request.form['time'], request.form['payment_mode'], float(request.form['amount']), request.form.get('description', entry.get('description', '')).strip()

        new_status = request.form.get('status', entry.get('status'))
        approver_select, approver_custom = request.form.get('approved_by_select', ''), request.form.get('approved_by_custom', '').strip()
        if 'status' in request.form:
            chosen_approver = approver_custom if approver_select == 'other' and approver_custom else (approver_select if approver_select and approver_select != 'other' else entry.get('approved_by', ''))
        ...
        if has_link:
            new_account_raw, new_account_name, new_nature = request.form.get('primary_account', 'main'), request.form.get('new_account_name', '').strip(), request.form.get('txn_nature', current_nature)
            new_account_type, new_primary_id, new_person_name = 'main', None, ''
            
            if new_account_raw == 'new_dasti':
                ref = db.collection('dasti_persons').document(); ref.set({'user_id': firm_id, 'name': new_account_name}); new_primary_id, new_account_type, new_person_name = ref.id, 'dasti', new_account_name
            elif new_account_raw == 'new_person':
                ref = db.collection('persons').document(); ref.set({'user_id': firm_id, 'name': new_account_name}); new_primary_id, new_account_type, new_person_name = ref.id, 'person', new_account_name
            elif new_account_raw.startswith('person_'):
                new_primary_id = new_account_raw.split('_', 1)[1]; new_account_type = 'person'; pd = db.collection('persons').document(new_primary_id).get().to_dict(); new_person_name = pd.get('name', '') if pd else ''
            elif new_account_raw.startswith('dasti_'):
                new_primary_id = new_account_raw.split('_', 1)[1]; new_account_type = 'dasti'; dd = db.collection('dasti_persons').document(new_primary_id).get().to_dict(); new_person_name = dd.get('name', '') if dd else ''

            base_txn = {'user_id': firm_id, 'date': date_val, 'time': time_val, 'payment_mode': mode, 'category': category, 'amount': amount, 'link_id': link_id, 'status': new_status, 'approved_by': approved_by, 'deleted': entry.get('deleted', 0), 'created_at': entry.get('created_at', time.time())}
            for coll, (did, _d) in linked_docs.items(): db.collection(coll).document(did).delete()

            if new_account_type == 'main':
                db.collection('transactions').add({**base_txn, 'description': desc, 'type': 'income' if new_nature == 'receive_cash' else 'expense'})
            ...
            doc_ref.update({'date': date_val, 'time': time_val, 'payment_mode': mode, 'category': category, 'amount': amount, 'status': new_status, 'approved_by': approved_by, 'description': desc, 'type': request.form.get('type', entry.get('type'))})
        return redirect(request.referrer or url_for('index'))
    return render_template_string(EDIT_TEMPLATE, entry=entry, table_name=table_name, categories=get_categories(firm_id), persons=persons, dasti_persons=dasti_persons, approvers=approvers, approver_names=approver_names, has_link=has_link, current_account_type=current_account_type, current_primary_id=current_primary_id, current_nature=current_nature, username=session['username'])
